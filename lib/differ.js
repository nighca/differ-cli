...
var isSimilar = require('./similarity').judge;
...
var diffByWords = function(line1, line2){
    if(
        line1.cnt.length >= config.lengthLimit ||
        line2.cnt.length >= config.lengthLimit
    ){
        return;
    }

    var prevEnd = 0,
        origin = line1.participledCnt || participle(line1.cnt),
        target = line2.participledCnt || participle(line2.cnt),
        originArr = [],
        targetArr = [];
    ...
    lines.forEach(function(line, i){
        ...
        if(
            ...
            if(!config.similarityJudge){
                ...
                var removed = lines.slice(i - removeNum + 1, i + 1),
                    inserted = lines.slice(i + 1, i + insertNum + 1);

                var j = 0;
                removed.forEach(function(removedLine){
                    for(var insertedLine; j < insertNum; j++){
                        insertedLine = inserted[j];
                        if(insertedLine){
                            var removedCnt = removedLine.participledCnt = participle(removedLine.cnt),
                                insertedCnt = insertedLine.participledCnt = participle(insertedLine.cnt);
                            if(insertedLine && isSimilar(removedCnt, insertedCnt)){
                                diffByWords(removedLine, insertedLine);
                                inserted[j] = null;
                                break;
                            }
                        }
...
var getOutput = function(cnt1, cnt2){
    ...
    for(var i = 0, l = linesDiff.length, line, start, end; i < l; i++){
        ...
        if(line.type !== 'normal'){
            if(curr && i <= curr.end){
                curr.end = Math.min(l - 1, i + config.aroundLineNum);
            }else{
                if(curr){
                    //console.log(curr);
                    blocks.push({
                ...
                curr = {
                    start: Math.max(0, i - config.aroundLineNum),
                    end: Math.min(l - 1, i + config.aroundLineNum)
                };
            }
        }
